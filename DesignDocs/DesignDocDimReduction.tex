...
\begin{itemize}
\item[Plan A:] Change PQL so that similar random effects are fused into a shared, single random effect.
\item[Plan B:] Keep the PQL optimization as it is, and add another step in between the two steps. At this time, the similar random effects are fused into a shared, single random effect.
  \end{itemize}
  ...
  I'm not sure, but it seems like Plan A is better than Plan B. It makes sense that we'd want to try fusing the random effects while making sure this simplification not ruin the fit of the model.
   ...
   In the currently used version of PQL, there is an optimization within an optimization. The inner optimization estimates the fixed effects and predicts the random effects. The outer optimization works on the variance components. \textbf{I believe that adding a penalty term to the inner loop will fuse the random effects.} We want the random effects to be similar to one other, so the distance (or the square of the distance, if we prefer a ridge-like penalty) between the random effects must be controlled. This is almost exactly pairwise fusion lasso (Petry et. al. 2011), except we are fusing \textit{random} effects instead of \textit{fixed} effects.
