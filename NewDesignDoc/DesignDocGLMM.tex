...
\begin{align}
\log \grave{f}(u)&=\log \left[ 1+u'D^* u/\zeta  \right]^{-(\zeta+q)/2}\\
&=-\left[ \dfrac{\zeta+q}{2} \right] \log (1+u'D^*u/\zeta)\\
&= -\left[ \dfrac{\zeta+q}{2} \right] \log \left(1+\sum_{t=1}^T \dfrac{u_t'u_t}{\zeta \nu_t}\right)
\end{align}

Rewritten in C, we need to pass in \texttt{u} as a pointer, \texttt{zeta} as an int, \texttt{myq} as an int array, \texttt{meow} as an int array, and \texttt{logft} as a pointer.
