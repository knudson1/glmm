...
\begin{enumerate}
\item Start by writing the binomial cumulant in R. Check to make sure this calculation is identical to that produced by bernoulli for some parameter value $\eta_i$ and for $ntrials=1$. Check for both $\eta$ positive and negative.
...
\item The second lowest level will then call binomial.glmm rather than bernoulli.glmm as the family. I will work my way up the levels incorporating ntrials. At the top, I will have a variable to set ntrials = 1 (for now). As I do each layer, I will make sure I get all the same answers from my salamanders problem (or some other bernoulli problem).
...
\item I will add checks for the model matrix stage.  See below.
...
Checks for the model matrix stage: 
\begin{enumerate}
