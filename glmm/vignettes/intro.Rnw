...
Finally, we specify the number of cores we would like to use for the value, gradient vector and Hessian matrix calculations, done in parallel, using \texttt{cores}. To read more about the \texttt{cores} argument, please read \href{https://cran.r-project.org/web/packages/glmm/vignettes/parallel.pdf}{"Model-Fitting in Parallel with the R package \texttt{glmm}"}
...
bernoulli.glmm, m = 10^4, debug = TRUE, cores = 2)
@
