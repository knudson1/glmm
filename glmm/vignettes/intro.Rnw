...
Finally, we specify the number of cores we would like to use for the parallel calculations using \texttt{cores}. To read more about the \texttt{cores} argument, please read \href{https://cran.r-project.org/web/packages/glmm/vignettes/parallel.pdf}{"Model-Fitting in Parallel with the R package \texttt{glmm}"}
