...
Finally, we specify the cluster we would like to use for the value, gradient vector and Hessian matrix calculations, done in parallel, using \texttt{cluster}. To read more about the \texttt{cluster} argument, please read \href{https://cran.r-project.org/web/packages/glmm/vignettes/parallel.pdf}{"Model-Fitting in Parallel with the R package \texttt{glmm}"}
