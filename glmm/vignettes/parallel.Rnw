...
Using the \texttt{names} command allows us to see the additional output that we can access, beyond the model summary. With the addition of parallel calculations, we also have the addition of the \texttt{cores} output. \texttt{cores} will tell you how many cores were used during the value, gradient vector and Hessian matrix calculations. 
...
To read about the model summary or the other additional output provided when using the \texttt{glmm} command, please read \href{https://cran.r-project.org/web/packages/glmm/vignettes/intro.pdf}{"An Introduction to Model-Fitting with the R package \texttt{glmm}"}
