...
Next, we illustrate how \texttt{weights} can be used to remove observations. A portion of one element of the \texttt{random} list is given to illustrate that the observations have been removed.
