...
varcomps.names = c("F", "M"), data = salamander, family.glmm =
...
sal <- glmm(Mate ~ 0 + Cross, random = list(~ 0 + Female, ~ 0 + Male),
 varcomps.equal = c( 1, 1), varcomps.names = c("Only Varcomp"), data = 
salamander, family.glmm = bernoulli.glmm, m = 10^4, debug = TRUE)
@
...
\begin{align*}
\log \left( \dfrac{\pi_i}{1-\pi_i} \right) &= \Sexpr{round(coef(sal)[1],4)} * I(Cross=R/R) + \Sexpr{round(coef(sal)[2],4)} * I(Cross=R/W)\\
 &+ \Sexpr{round(coef(sal)[3],4)} * I(Cross=W/R)
+ \Sexpr{round(coef(sal)[4],4)} * I(Cross=W/W) + u_i^F+u_i^M\\
