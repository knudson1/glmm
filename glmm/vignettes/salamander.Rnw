%% How to compile by hand, not in vignette: 
% R --vanilla
% library(knitr)
% knit("salamDemo.Rnw")
...
%\VignetteKeywords{glmm, maximum likelihood, mle, Monte Carlo, likelihood approximation, likelihood based inference}
...
data=salamander,family.glmm=bernoulli.glmm,m=10^4,debug=TRUE)
@
...
sal<-glmm(Mate~0+Cross, random=list(~0+Female,~0+Male), varcomps.equal=c(1,1),
varcomps.names=c("Only Varcomp"), data=salamander,
...
The next  output is  \texttt{umat}, a matrix of the random effects generated from the importance sampling distribution. The matrix has $m$ rows. The number of columns is equal to the total number of random effects in the model.\\
